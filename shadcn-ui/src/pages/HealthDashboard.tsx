...
export default function HealthDashboard() {
  ...
  // React Query hooks
  const { stats, visualizations, isLoading: isDashboardLoading } = useDashboard();
  const { appointments, isLoading: isAppointmentsLoading } = useAppointments();
  const { healthRecords, createHealthRecord, deleteHealthRecord, isCreating: isCreatingRecord } = useHealthRecords();
  const { notifications, unreadCount, markAsRead, markAllAsRead, deleteNotification, createNotification, isCreating: isCreatingNotification } = useNotifications();
  const { medications, isLoading: isMedicationsLoading } = useMedications();
  const { vitals, isLoading: isVitalsLoading } = useVitals();
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => navigate('/vitals')}>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-800">{upcomingAppointmentsList.length}</p>
              </div>
        ...
        <Card className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => navigate('/medications')}>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">Active Medications</p>
                <p className="text-3xl font-bold text-gray-800">{activePrescriptions}</p>
              </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={handleUploadHealthRecord} className="space-y-4">
            ...
            <DialogFooter>
              <Button 
                type="submit"
                disabled={!uploadFormData.title || isCreatingRecord}
                className="w-full"
              >
                {isCreatingRecord ? (
                  <>
